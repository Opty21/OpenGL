...
public:
    ...
    void createFromMatrix(vector<vector<char>> matrix,int scale){
        OBJ_vertices.clear();
        OBJ_vertices.shrink_to_fit();
        OBJ_normals.clear();
        OBJ_normals.shrink_to_fit();
        OBJ_uvs.clear();
        OBJ_uvs.shrink_to_fit();
        matModel = glm::mat4(1.0);
        initPos = glm::vec3(0,0,0);
        initRotation = glm::vec3(0,0,0);
        int hor=0;
    ...
    void translateAllInstantiated(glm::vec3 trVal,bool y){

        for(int i =0 ; i< matModelArray.size();i++){
            //matModelArray[i] = glm::translate(matModelArray[i],trVal);
            // matModelArray[i][3].x += trVal.x;
            // matModelArray[i][3].y += trVal.y;
            // matModelArray[i][3].z += trVal.z;
            matModelArray[i][3] += glm::vec4(trVal,0);
           if(y) matModelArray[i][3] += glm::vec4(-prevTranslate,0);
...
class Collidable: public CMesh{
public:
    float colliderRadius = _tileScale*1.5f/2.0f;
    float colliderRadiusInteract = _tileScale*1.7f/2.0f;
    bool currentlyMoving = false;
    float currMoveValue = 0;
    glm::vec3 currMoveDirection = glm::vec3(0,0,0);
    tuple <bool,bool> checkForCollision(int i,glm::vec3 otherPos){
        bool p1 = false; //INTERACTION COLLIDER
        bool p2 = false; //MESH BLOCK COLLIDER

        if(glm::distance(glm::vec4(otherPos,0),matModelArray[i][3]) < colliderRadiusInteract){
            p1 = true;
            if(currentlyMoving) p1 = false;
            if(glm::distance(glm::vec4(otherPos,0),matModelArray[i][3]) < colliderRadius) p2 = true;
        }
