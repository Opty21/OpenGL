...
public:
	...
	float getAltitute(glm::vec2 point,float py)
	{
		...
		for (auto it = triangles.begin(); it != triangles.end(); it++)
		{
			...
			{
				collTriangle = &(*it);
				float tempDist =  abs(py - collTriangle->calulateY(point));
		...
		if (collTriangle)
		{
			return nearsetTriangle->calulateY(point);
		}
		else
		{
