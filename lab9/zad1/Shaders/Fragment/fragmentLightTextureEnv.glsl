...
{
	//Reflection
	vec3 DirReflection = normalize(fragPos - cameraPosition);
	vec3 Reflection = reflect(DirReflection, inNormal);
	...
	vec3 Refraction = refract(Dir, inNormal, Refraction_Coeff);
	...
	vec3 combine = fragColor/3.0 + 2.0*ReflectionColor/4.0 + 2.0*RefractionColor/4.0;
	outColor = vec4(combine, 1.0);
}
